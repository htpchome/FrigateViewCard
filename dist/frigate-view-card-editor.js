...
}`,Wo=Object.freeze({"border-bottom":"none","border-top":"1px solid var(--divider-color)","box-sizing":"border-box",height:`calc(var(--header-height, 56px) + ${ta}px)`,"padding-top":`${ta/2}px`,"padding-bottom":`${ta/2}px`}),Fo=Symbol.for("frigate-view-card.dashboard-navbar-customization"),ei=e=>String(e||"").trim().toLowerCase().replace(/^custom:/,""),eo=(e,t)=>String(e?.path||"").trim().replace(/^\/+|\/+$/g,"")||String(t),to=(e,t="frigate-view-card")=>{const a=ei(t),i=[];if(!a||!Array.isArray(e?.views))return{cards:i,claimants:[],owner:null,conflicts:[]};let r=0;e.views.forEach((n,s)=>{const d=new Set,c=(h,p=0)=>{if(!(!h||typeof h!="object"||p>30)&&!d.has(h)){if(d.add(h),!Array.isArray(h)&&ei(h.type)===a){i.push({config:h,cardOrder:r,view:n,viewIndex:s,viewName:eo(n,s),viewTitle:String(n?.title||"").trim()||`Page ${s+1}`}),r+=1;return}Object.values(h).forEach(g=>c(g,p+1))}};c(n)});const o=i.filter(({config:n})=>n?.mobile_view_ha_navbar_bottom===!0&&n?.mobile_view_ha_navbar_dashboard===!0);return{cards:i,claimants:o,owner:o[0]||null,conflicts:o.slice(1)}},ao=({dashboardConfig:e,sourceConfig:t=null,requested:a=!1,cardTag:i="frigate-view-card",currentViewName:r=""}={})=>{const o=to(e,i),n=o.cards.find(({config:h})=>h===t),s=o.claimants.filter(({viewName:h})=>h===r),d=!!o.owner&&(o.owner===n||!n&&a&&o.owner.viewName===r&&s.length===1),c=a&&(!o.owner||d);return{...o,requested:a,isOwner:c,locked:!!o.owner&&!d,conflict:a&&!!o.owner&&!c}},Go=Object.freeze(["transform","transition","will-change","backface-visibility"]),Uo=Object.freeze(["overflow-x","overscroll-behavior-x"]),Zo=["a","button","input","select","textarea","label","summary","video","audio","canvas","[contenteditable]","[draggable='true']","[role='button']","[role='slider']","[role='spinbutton']","[data-no-swipe]","ha-tabs","ha-tab-group","ha-tab-group-tab","paper-tabs","paper-tab","ha-slider","ha-control-slider","paper-slider","mwc-slider","ha-map","hui-map-card","google-map","swipe-card","swiper-container",".swiper",".swiper-container",".swiper-wrapper",".swiper-slide","#live-stage","#viewer","#recording-scrub",".recording-scrub-track",".recording-segment-handle",".live-resize-grip",".popup-view-resize-grip",".popup-carousel",".circle-pad",".wide-timeline-shell",".wide-timeline-resize-handle",".card-view-scroller",".linked-light-brightness-popover",".bubble-pop-up",".bubble-pop-up-container",".bubble-backdrop","#filter-panel","#cal-panel"].join(","),Ko=new Set([k.dashboardWide,k.landingDashboard]),Yo=Symbol.for("frigate-view-card.dashboard-swipe-navigation"),io=e=>{if(!e)return null;if(e.parentNode)return e.parentNode;const t=e.getRootNode?.();return t&&t!==e?t.host||null:e.host||null},Pt=e=>String(e||"").trim().replace(/^\/+|\/+$/g,""),ro=e=>{const t=String(e||"").trim().replace(/\/+$/g,"");return t?t.startsWith("/")?t:`/${t}`:""},oo=(e,t)=>Pt(e?.path)||String(t),ti=e=>String(e||"").trim().toLowerCase().replace(/^custom:/,""),so=(e,t)=>ti(e?.type)===t,no=(e,t="frigate-view-card")=>{const a=ti(t),i=e?.views;if(!a||!Array.isArray(i))return[];const r=[];let o=0;return i.forEach((n,s)=>{const d=new Set,c=(h,p=0)=>{if(!(!h||typeof h!="object"||p>30)&&!d.has(h)){if(d.add(h),!Array.isArray(h)&&so(h,a)){r.push({config:h,cardOrder:o,view:n,viewIndex:s,viewName:oo(n,s),viewTitle:String(n?.title||"").trim()||`Page ${s+1}`}),o+=1;return}Object.values(h).forEach(g=>c(g,p+1))}};c(n)}),r},lo=(e,t="frigate-view-card")=>{const a=no(e,t),i=a.filter(({config:r})=>r?.ha_dashboard_swipe_navigation_owner===!0);return{cards:a,claimants:i,owner:i[0]||null,conflicts:i.slice(1)}},ai=(e,t=globalThis.document)=>{let a=e;for(let n=0;a&&n<12;n+=1){if(String(a.tagName||"").toUpperCase()==="HA-PANEL-LOVELACE")return a;a=io(a)}const r=t?.querySelector?.("home-assistant")?.shadowRoot?.querySelector?.("home-assistant-main")?.shadowRoot,o=r?.querySelector?.("partial-panel-resolver");return o?.querySelector?.("ha-panel-lovelace")||o?.shadowRoot?.querySelector?.("ha-panel-lovelace")||r?.querySelector?.("ha-panel-lovelace")||null},co=({panel:e,huiRoot:t,windowRef:a})=>{const i=ro(e?.route?.prefix||t?.route?.prefix||t?._route?.prefix),r=String(a?.location?.pathname||"");if(i&&r.startsWith(`${i}/`))return Pt(r.slice(i.length));const o=Pt(e?.route?.path);return o||Pt(r).split("/").at(-1)||""},ii=(e={})=>co(e),Xo=Object.freeze(["camera.doorbell","camera.front_door","camera.driveway","camera.garage","camera.backyard"]),ho=e=>{let t=[];return Array.isArray(e?.cameras)?t=e.cameras:e?.camera_entity&&(t=[{entity:e.camera_entity,name:e.title||"",connection_type:N}]),Zt(t.map(a=>Xa(a,{fallbackName:""})).filter(a=>a.entity),me)},po=e=>{const t=e&&typeof e=="object"?{...e}:{},a=ho(t);t.hidden_tabs=Array.isArray(t.hidden_tabs)?t.hidden_tabs.map(o=>o==="reviews"?"alerts":o).filter(o=>Te.includes(o)):[...Ge],delete t.camera_entity,t.title=q(t.title).trim()||_t,t.subtitle=q(t.subtitle).trim()||Ht,t.display_title=t.display_title!==!1,t.display_subtitle=t.display_subtitle!==!1,t.display_logo=t.display_logo!==!1,t.display_version=t.display_version!==!1,t.theme=t.theme==="custom"?"custom":"default",t.theme_custom=ft(t.theme_custom),t.theme_custom_defaults=Lt(t.theme_custom_defaults),t.shadows=t.shadows!==!1,t.borders=t.borders===!0,t.rounded_corners=t.rounded_corners!==!1,t.outer_shadows=t.outer_shadows!==!1,t.stream_height=Ye(t.stream_height),t.stream_height_unit=Xe(t.stream_height_unit),t.col_left_width_pct=xt(t.col_left_width_pct),t.realtime_poll_seconds=Re.includes(Number(t.realtime_poll_seconds))?Number(t.realtime_poll_seconds):5,t.snapshot_update_seconds=$(t.snapshot_update_seconds,Ne,Ae),t.mobile_poll_battery_saver=t.mobile_poll_battery_saver===!0,t.event_pre_post_roll_enabled=t.event_pre_post_roll_enabled===!0,t.favorites_mixed_cameras=t.favorites_mixed_cameras!==!1,t.slideshow_rotation_enabled=t.slideshow_rotation_enabled===!0,t.slideshow_rotation_seconds=Ie.includes(Number(t.slideshow_rotation_seconds))?Number(t.slideshow_rotation_seconds):30,t.slideshow_alert_hold_seconds=$(t.slideshow_alert_hold_seconds,je,Math.round(Me/1e3)),t.grid_mode_enabled=t.grid_mode_enabled===!0,t.grid_order=ue(t.grid_order,a),t.grid_live_view_enabled=t.grid_live_view_enabled!==!1,t.grid_alert_hold_seconds=$(t.grid_alert_hold_seconds,We,Math.round(Le/1e3)),t.mobile_view_page_enabled=t.mobile_view_page_enabled!==!1,t.mobile_view_rotate_to_fullscreen=t.mobile_view_rotate_to_fullscreen===!0,t.mobile_view_dashboard_background=t.mobile_view_dashboard_background!==!1,t.mobile_view_header_overlay=t.mobile_view_header_overlay===!0,t.mobile_view_outer_border=t.mobile_view_outer_border===!0,t.mobile_view_ha_navbar_bottom=t.mobile_view_ha_navbar_bottom===!0,t.mobile_view_ha_navbar_stack_tabs=t.mobile_view_ha_navbar_stack_tabs===!0,t.mobile_view_ha_navbar_dashboard=t.mobile_view_ha_navbar_dashboard===!0,t.ha_dashboard_swipe_navigation_owner=t.ha_dashboard_swipe_navigation_owner===!0,t.ha_dashboard_swipe_navigation=Ke(t.ha_dashboard_swipe_navigation),t.ha_dashboard_swipe_include_other_cards=t.ha_dashboard_swipe_include_other_cards===!0,t.ha_dashboard_swipe_include_subviews=t.ha_dashboard_swipe_include_subviews===!0,t.ha_dashboard_swipe_mouse_enabled=t.ha_dashboard_swipe_mouse_enabled===!0,t.preview_page_enabled=t.preview_page_enabled===!0,t.preview_page_live_cameras=t.preview_page_live_cameras===!0,t.preview_page_live_cameras_mobile=t.preview_page_live_cameras_mobile===!0,t.preview_page_show_title_bars=t.preview_page_show_title_bars!==!1,t.preview_page_alert_live_duration_seconds=$(t.preview_page_alert_live_duration_seconds,Fe,10),t.single_view_start_mode=ne(t.single_view_start_mode),t.wide_view_start_mode=ne(t.wide_view_start_mode),t.card_view_start_mode=Kt(t.card_view_start_mode),t.single_view_alert_takeover=t.single_view_alert_takeover===!0,t.wide_view_page_enabled=t.wide_view_page_enabled===!0||t.wide_view===!0,t.wide_view_live_cameras=t.wide_view_live_cameras===!0,t.wide_view_alert_takeover=t.wide_view_alert_takeover===!0,t.wide_view_timeline_enabled=t.wide_view_timeline_enabled===!0,t.wide_view_timeline_default_open=t.wide_view_timeline_default_open===!0,t.wide_view_timeline_default_scale=St(t.wide_view_timeline_default_scale),t.card_view_page_enabled=t.card_view_page_enabled===!0,t.card_view_alert_takeover=t.card_view_alert_takeover===!0,t.card_view_standalone=t.card_view_page_enabled&&t.card_view_standalone===!0,t.card_view_media_drawer_enabled=t.card_view_media_drawer_enabled!==!1,t.card_view_view_mode=at(t.card_view_view_mode,{legacyDrawerDefaultOpen:t.card_view_drawer_default_open,legacyVideoPanelOnly:t.card_view_video_panel_only}),delete t.card_view_drawer_default_open,delete t.card_view_media_drawer_type,delete t.card_view_video_panel_only,t.card_view_hide_camera_name=t.card_view_hide_camera_name!==!1,t.landing_page=oe(t.landing_page),t.card_view_standalone&&(t.landing_page=_.cardView),t.mobile_page=xa(t,t.mobile_page);const i=se(t,O.desktop);i.includes(t.landing_page)||(t.landing_page=i[0]||_.singleView),t.ha_dashboard_swipe_pages=ut(t,O.desktop),t.ha_dashboard_swipe_mobile_pages=vt(t),t.grid_rotation_seconds=Be.includes(Number(t.grid_rotation_seconds))?Number(t.grid_rotation_seconds):30;const r=parseInt(t.window_hours,10);return t.event_days=At(t.event_days??t.window_days,Number.isFinite(r)&&r>0?Math.max(1,Math.ceil(r/24)):he),t.alerts_reviews_days=At(t.alerts_reviews_days,pe),delete t.window_days,delete t.window_hours,delete t.wide_view,{...t,cameras:a}},ri=new Set(["","default","backend-selected"]),oi=Object.freeze(["dark-primary-color","--dark-primary-color"]),Dt=e=>String(e||"").trim().toLowerCase(),mo=(e,t)=>{const a=e?.themes;if(!a||typeof a!="object")return null;const i=Dt(t),r=Object.keys(a).find(n=>Dt(n)===i);if(!r)return null;const o=a[r];return o&&typeof o=="object"?o:null},si=(e,t)=>t.some(a=>Object.prototype.hasOwnProperty.call(e||{},a)&&String(e[a]??"").trim()!==""),_o=(e,t)=>si(e,oi)||si(e?.modes?.[t],oi),uo=(e,{mode:t}={})=>{const a=e?.themes||{},{darkMode:i=!1}=a,r=t==="dark"||t==="light"?t:i===!0?"dark":"light",o=String(a.theme||e?.selectedTheme||"default").trim(),n=String(r==="dark"?a.default_dark_theme||a.default_theme||"default":a.default_theme||"default").trim(),s=ri.has(Dt(o))?n:o,d=ri.has(Dt(s))?"default":"custom",c=d==="custom"?mo(a,s):null;return{mode:r,source:d,deriveDarkPrimary:d==="custom"&&c!==null&&!_o(c,r)}},rt=Object.freeze({enterStandalone:"enter-card-view-standalone",revertStandaloneDraft:"revert-card-view-standalone-draft",navigate:"navigate",commit:"commit",reset:"reset"}),ni=(e,t)=>{const a=String(t?.attributes?.friendly_name||"").trim();if(a)return a;const i=String(e||"").replace(/^light\./,"").trim();return i?i.split(/[_\s-]+/).filter(Boolean).map(r=>`${r.charAt(0).toUpperCase()}${r.slice(1)}`).join(" "):"Light"},vo=Object.freeze({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}),L=e=>String(e??"").replace(/[&<>"']/g,t=>vo[t]),P=L,li=e=>{const t=String(e||"").trim().replace(/^v/i,"");if(!t)return null;const a=t.match(/^\d+(?:\.\d+)*/)?.[0]||"";return a?a.split(".").map(i=>Number(i)):null},go=(e,t)=>{const a=li(e),i=li(t);if(!a||!i)return null;const r=Math.max(a.length,i.length);for(let o=0;o<r;o+=1){const n=a[o]||0,s=i[o]||0;if(n<s)return-1;if(n>s)return 1}return 0},bo=({currentVersion:e="",recommendedVersion:t=""}={})=>go(e,t)!==-1?"":`Home Assistant ${e} is below the recommended ${t}.`,fo=({currentVersion:e="",recommendedVersion:t=""}={})=>{const a=String(e||"").trim();if(!a)return{visible:!1,status:"unavailable",label:""};const i=bo({currentVersion:a,recommendedVersion:t});return i?{visible:!0,status:"warning",label:i}:{visible:!0,status:"current",label:`Home Assistant ${a}`}},wo=({installed:e=null}={})=>e===!0?{visible:!0,status:"current",label:"Frigate integration is installed."}:e===!1?{visible:!0,status:"error",label:"Frigate integration is not installed in Home Assistant."}:{visible:!1,status:"unavailable",label:""},yo=e=>{const t=e?.config?.components;return Array.isArray(t)?t.includes("frigate"):t instanceof Set?t.has("frigate"):null},xo=Object.freeze(new Set(["camera-modal-entity","camera-modal-secondary-entity","camera-modal-light-entity","camera-modal-light-icon","camera-modal-light-entity-2","camera-modal-light-icon-2","camera-modal-connection-type"])),So="dirtyState",di="frigate-view-card-editor",Co=200,ci=3e4,xe=L,ko=e=>{const t=Number(e);return Number.isFinite(t)&&t>=60&&t%60===0?`${t/60} min`:`${t} sec`},Se=e=>e.map(t=>({value:t,label:ko(t)})),ot=({name:e,options:t,selectedValue:a,compact:i=!1})=>{const r=String(a??""),o=xe(e),n=t.some(({description:s})=>!!String(s||"").trim());return`<div class="editor-choice-chips${i?" editor-choice-chips--compact":""}${n?" editor-choice-chips--detailed":""}">
    ${t.map(({value:s,label:d,description:c="",disabled:h=!1})=>{const p=xe(s),g=xe(d),E=xe(c);return`<label class="editor-choice-chip">
    ...
    ${t.map(({value:o,label:n,disabled:s=!1,disabledReason:d=""})=>{const c=xe(o),h=xe(n),p=P(d),g=s===!0;return`<label class="theme-scope-opt card-view-start-opt" data-disabled-guidance="${p}"${g&&p?` title="${p}"`:""}>
      ...
      <div class="setting-content">
        ...
        <div class="settings-more-slot">
          ...
          ${Ce?`<label class="editor-swipe-choice-footer"><span>Swipe to Subviews</span><ha-switch data-ha-dashboard-swipe-include-subviews="${P(l)}" ${this._config?.ha_dashboard_swipe_include_subviews?"checked":""} ${g&&H?"":"disabled"}></ha-switch></label>`:""}
        </div>`}).join(""),C=[k.dashboardWide,k.insideCard].includes(p),be=new Set(se(this._config,O.desktop)),F=oe(this._config?.landing_page),st=new Set(ut(this._config,O.desktop)),ee=new Set(se(this._config,O.mobile)),ze=De(this._config),fe=new Set(vt(this._config)),ce={[_.preview]:"Preview",[_.singleView]:"Single View",[_.mobileView]:"Mobile View",[_.wideView]:"Wide View",[_.cardView]:"Card View"},M=Rt.map(l=>{const m=l===F,y=!g||!be.has(l)||m;return`<label class="editor-choice-chip">
          <input class="editor-choice-chip-input" type="checkbox" name="ha_dashboard_swipe_pages" value="${P(l)}" ${st.has(l)?"checked":""} ${m?'data-dashboard-swipe-landing="true"':""} ${y?"disabled":""}>
          <span class="editor-choice-chip-body">
            <span class="editor-choice-chip-indicator" aria-hidden="true"></span>
            <span class="editor-choice-chip-text">${L(ce[l])}</span>
          </span>
        </label>`}).join(""),G=Nt.map(l=>{const m=l===ze,y=!g||!ee.has(l)||m;return`<label class="editor-choice-chip">
        ...
        </label>`}).join(""),X=Ut(t),J=Va(t),T=ue(this._config?.grid_order,t),Q=new Map(J.map(l=>[l.entity,l])),j=T.mode===W.custom?T.included.length:X,Oe=X<me,nt=this._timezoneDisplay(),b=new Set(this._normalizeHiddenTabs(this._hiddenTabsDraft??this._config?.hidden_tabs));this._ensureThemeDraftCache();const D=this._config?.theme==="custom"?"custom":"default",V=this._activeThemeModeKey(),te=Wt(this._config?.theme_custom,V),pi=Lt(this._config?.theme_custom_defaults),mi=te.scope,_i=te.overrides,ui=pi[V]||{},vi=this._themeDraftCache[V]||{},aa=Ye(this._config?.stream_height),ia=Xe(this._config?.stream_height_unit),ra=xt(this._config?.col_left_width_pct),gi=St(this._config?.wide_view_timeline_default_scale),bi=Re.includes(Number(this._config?.realtime_poll_seconds))?Number(this._config.realtime_poll_seconds):5,fi=$(this._config?.snapshot_update_seconds,Ne,Ae),wi=$(this._config?.preview_page_alert_live_duration_seconds,Fe,Math.round(rr/1e3)),yi=$(this._config?.slideshow_alert_hold_seconds,je,Math.round(Me/1e3)),xi=$(this._config?.slideshow_rotation_seconds,Ie,30),Si=Be.includes(Number(this._config?.grid_rotation_seconds))?Number(this._config.grid_rotation_seconds):30,Vt=(l,m)=>Eo({name:l,selectedValue:m,gridEnabled:this._config?.grid_mode_enabled===!0,slideshowEnabled:this._config?.slideshow_rotation_enabled===!0}),Ci=Vt("single_view_start_mode",this._config?.single_view_start_mode),ki=Vt("wide_view_start_mode",this._config?.wide_view_start_mode),Ei=Vt("card_view_start_mode",this._config?.card_view_start_mode),oa=at(this._config?.card_view_view_mode),Ai=[{value:K.videoOnly,label:"Video Only"},{value:K.bottomPanelOpen,label:"Bottom Panel Open"},{value:K.bottomPanelClosed,label:"Bottom Panel Closed"}].map(({value:l,label:m})=>`<label class="theme-scope-opt card-view-start-opt">
          <input class="card-view-start-input" type="radio" name="card_view_view_mode" value="${l}" ${oa===l?"checked":""}>
          <span>${m}</span>
        </label>`).join(""),sa=l=>l===_.mobileView?"Mobile":l===_.preview?"Preview":l===_.wideView?"Wide View":l===_.cardView?"Card View":"Single View",Mi=se(this._config,O.desktop).map(l=>({value:l,label:sa(l)})),Li=this._standaloneLandingPageRoutes().map(l=>({value:l,label:sa(l)})),Ti={[f.mobile]:"Mobile",[f.card]:"Card View",[f.previewMobile]:"Preview + Mobile",[f.previewSingle]:"Preview + Single View",[f.single]:"Single View"},Pi=It(this._config).map(l=>({value:l,label:Ti[l]})),$e=(l,m)=>`<ha-formfield label="${m}">
          <ha-switch data-active-tab="${l}" ${b.has(l)?"":"checked"}></ha-switch>
        </ha-formfield>`,Di=qt.map(l=>{const m=l.key,y=this._themeDefaultHex(m,V),H=ve(_i[m]),Z=ve(vi[m]),Ce=H||Z||y,ie=ui[m]===!0,He=ie?y:Ce,pt=!ie&&He!==y;return`
        <div class="theme-custom-row" data-theme-row="${m}">
          ...
          <div class="theme-color-wrap">
            <input class="theme-color-input" type="color" data-theme-color="${m}" value="${He}" ${ie?"disabled":""}>
            <button
              type="button"
        ...
        </div>`}).join(""),Vi=[{value:"light",label:"Light",icon:"mdi:white-balance-sunny"},{value:"dark",label:"Dark",icon:"mdi:weather-night"},{value:"both",label:"Both",icon:"mdi:theme-light-dark"}].map(({value:l,label:m,icon:y})=>{const H=mi===l;return`<button
          type="button"
        ...
        ><ha-icon icon="${y}"></ha-icon><span>${m}</span></button>`}).join(""),zi=t.map((l,m)=>`
      <div class="cam-row" draggable="true" data-row="${m}">
        ...
        <div><div class="cam-name">${L(this._cameraLabel(l))}</div><div class="cam-meta">${L(this._cameraMetaLabel(l))}</div></div>
      ...
      </div>`).join(""),Oi=T.included.map((l,m)=>{const y=Q.get(l);return y?`${m%4===0?`<div class="grid-order-heading">Grid ${Math.floor(m/4)+1}</div>`:""}
          ...
          </div>`:""}).join(""),$i=T.excluded.map(l=>{const m=Q.get(l);return m?`<div class="grid-order-excluded-row" data-grid-order-entity="${P(l)}">
          <div class="grid-order-camera-copy">
      ...
      <div class="grid-order-custom camera-group-fields" ${T.mode===W.custom?"":"hidden"}>
        <div class="grid-order-sections">
          ${Oi||'<div class="cam-helper">No cameras are currently included in Grid mode.</div>'}
        </div>
      ...
      <div class="section">
        ...
        <div class="cam-wrap" id="cam-list">${zi}</div>
      ...
      </div>`,na=q(this._config?.title||_t),la=q(this._config?.subtitle||Ht),Ri=`
      <div class="environment-version-summary">
        <div class="card-version-status" id="card-version-status" data-update-status="unavailable">
          ...
          <div class="card-version-copy">
            <strong>${Ee}</strong>
            <span>Version v${L(tr)} <span aria-hidden="true">\u2022</span> <span id="card-version-update-status" role="status" aria-live="polite">Update status unavailable</span></span>
      ...
      <div class="text-display-field">
        <div class="text-display-row">
          <div class="limited-text-input">
            <ha-input label="Title" name="title" id="title" type="text" maxlength="${le}" value="${P(na)}" placeholder="${P(_t)}"></ha-input>
            <span class="limited-text-counter" id="title-counter" aria-hidden="true">${na.length}/${le}</span>
          </div>
      ...
      <div class="text-display-field">
        <div class="text-display-row">
          <div class="limited-text-input">
            <ha-input label="Subtitle" name="subtitle" id="subtitle" type="text" maxlength="${le}" value="${P(la)}" placeholder="${P(Ht)}"></ha-input>
            <span class="limited-text-counter" id="subtitle-counter" aria-hidden="true">${la.length}/${le}</span>
          </div>
      ...
      <div class="section">
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div style="display:flex;flex-direction:column;gap:6px;max-width:460px">
            ...
            <div class="field-helper">Adds ${ir} seconds before and after alert and clip playback or downloads. Requires Frigate recordings.</div>
          </div>
        </div>
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div style="display:flex;flex-direction:column;gap:6px;max-width:460px">
      ...
      <div class="section">
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start">
          <div class="editor-choice-field editor-choice-field--fit" id="realtime_poll_seconds" role="radiogroup" aria-label="Fallback Update Check">
            ...
            ${ge({name:"realtime_poll_seconds",options:Se(Re),selectedValue:bi})}
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div id="snapshot_update_row" style="min-width:210px;display:flex;flex-direction:column;gap:6px;width:100%">
            <div class="editor-choice-field" id="snapshot_update_seconds" role="radiogroup" aria-label="Snapshot Refresh">
              ...
              ${ge({name:"snapshot_update_seconds",options:Se(Ne),selectedValue:fi})}
            </div>
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div id="preview_alert_live_duration_row" style="min-width:210px;display:flex;flex-direction:column;gap:6px;width:100%">
            <div class="editor-choice-field" id="preview_page_alert_live_duration_seconds" role="radiogroup" aria-label="Alert Live Duration">
              ...
              ${ge({name:"preview_page_alert_live_duration_seconds",options:Se(Fe),selectedValue:wi})}
            </div>
        ...
        <div id="theme-custom-panel" class="theme-custom-panel" ${D==="custom"?"":"hidden"}>
          <div class="theme-custom-body">
            <div class="theme-custom-scope">
              ...
              <div class="theme-scope-seg" role="radiogroup" aria-label="Custom theme modes">${Vi}</div>
            </div>
            ${Di}
          </div>
        </div>
      ...
      <div class="section">
        ...
        <div class="card-height-control">
          ...
          <div class="editor-choice-field editor-choice-field--compact" id="stream_height_unit" role="radiogroup" aria-label="Card height unit">
            ${ot({name:"stream_height_unit",options:[{value:"%",label:"%"},{value:"dvh",label:"dvh"}],selectedValue:ia,compact:!0})}
          </div>
        </div>
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start">
          ...
          <div id="slideshow_rotation_row" style="display:${this._config?.slideshow_rotation_enabled?"flex":"none"};flex:1 1 100%;width:100%;flex-direction:column;gap:6px">
            <div class="editor-choice-field editor-choice-field--single-row" id="slideshow_rotation_seconds" role="radiogroup" aria-label="Camera Rotation Interval">
              ...
              ${ge({name:"slideshow_rotation_seconds",options:Se(Ie),selectedValue:xi})}
            </div>
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div id="slideshow_alert_hold_row" style="min-width:210px;display:flex;flex-direction:column;gap:6px;width:100%">
            <div class="editor-choice-field" id="slideshow_alert_hold_seconds" role="radiogroup" aria-label="Alert Hold Duration">
              ...
              ${ge({name:"slideshow_alert_hold_seconds",options:Se(je),selectedValue:yi})}
            </div>
      ...
      </div>`,Wi=$(this._config?.grid_alert_hold_seconds,We,Math.round(Le/1e3)),Fi=`
      <div class="section">
        <div class="layout-row">
      ...
      <div class="section timeline-dependent-section" id="wide-timeline-default-scale-row" style="${this._config?.wide_view_page_enabled&&this._config?.wide_view_timeline_enabled?"":"display:none"}">
        <div class="editor-choice-field" id="wide_view_timeline_default_scale" role="radiogroup" aria-label="Initial Timeline Range">
          ...
          ${ge({name:"wide_view_timeline_default_scale",options:La.map(l=>({value:l,label:`${l} hour${l===1?"":"s"}`})),selectedValue:gi})}
        </div>
      ...
      <div class="section">
        ...
        <div class="field-helper">Only one ${Ee} can control swipe navigation per dashboard.</div>
      ...
      <div class="card-view-page-options" id="card-view-page-options" style="${this._config?.card_view_page_enabled?"":"display:none"}">
        ...
        <div class="section">
          <div class="editor-choice-field" role="radiogroup" aria-label="View Mode">
            ...
            <div class="theme-scope-seg card-view-start-seg card-view-mode-seg">${Ai}</div>
          </div>
          <div class="field-helper">Starts with video only, or with the activity panel open or closed.</div>
        </div>
        <div class="card-view-video-only-options" id="card-view-video-only-options" style="${oa===K.videoOnly?"":"display:none"}">
          <div class="section">
            <div class="layout-row">
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start">
          ...
          <div id="grid_rotation_row" style="display:${this._config?.grid_mode_enabled&&j>4?"flex":"none"};flex:1 1 100%;width:100%;flex-direction:column;gap:6px">
            <div class="editor-choice-field editor-choice-field--single-row" id="grid_rotation_seconds" role="radiogroup" aria-label="Grid Rotation Interval">
              ...
              ${ge({name:"grid_rotation_seconds",options:Se(Be),selectedValue:Si})}
            </div>
        ...
        <div class="layout-row" style="align-items:flex-start;gap:12px;flex-wrap:wrap;justify-content:flex-start;margin-top:12px">
          <div id="grid_alert_hold_row" style="min-width:210px;display:flex;flex-direction:column;gap:6px;width:100%">
            <div class="editor-choice-field editor-choice-field--single-row" id="grid_alert_hold_seconds" role="radiogroup" aria-label="Grid Alert Hold Duration">
              ...
              ${ge({name:"grid_alert_hold_seconds",options:Se(We),selectedValue:Wi})}
            </div>
            <div class="field-helper">How long an alerted tile stays highlighted and, when needed, live.</div>
          </div>
        </div>
      </div>`,U=this._activeSettingsPanelId??null,Ji=`
      <div class="settings-container">
        ${this._renderSettingsPanel({id:"camera",title:"Camera Settings",icon:"mdi:camera",content:qi,active:U==="camera"})}
        ${this._renderSettingsPanel({id:"general",title:"General Settings",icon:"mdi:cog",content:Ri,active:U==="general"})}
        ${this._renderSettingsPanel({id:"theme",title:"Theme Settings",icon:"mdi:palette",content:Ni,active:U==="theme"})}
        ${this._renderSettingsPanel({id:"layout",title:"Layout Settings",icon:"mdi:angle-right",content:Ii,active:U==="layout"})}
        ${this._renderSettingsPanel({id:"slideshow",title:"Slideshow Settings",icon:"mdi:presentation-play",content:Bi,active:U==="slideshow"})}
        ${this._renderSettingsPanel({id:"gridview",title:"Grid Mode Settings",icon:"mdi:view-grid-outline",content:Xi,active:U==="gridview"})}
        ${this._renderSettingsPanel({id:"preview",title:"Preview Page",icon:"mdi:view-grid",content:ji,active:U==="preview"})}
        ${this._renderSettingsPanel({id:"singleview",title:"Single View Page",icon:z.singleView,content:Fi,active:U==="singleview"})}
        ${this._renderSettingsPanel({id:"wideview",title:"Wide View Page",icon:"mdi:view-split-vertical",content:Gi,active:U==="wideview"})}
        ${this._renderSettingsPanel({id:"cardview",title:"Card View Page",icon:z.cardView,content:Ki,active:U==="cardview"})}
        ${this._renderSettingsPanel({id:"mobileview",title:"Mobile View Page",icon:"mdi:cellphone",content:Ui,active:U==="mobileview"})}
        ${this._renderSettingsPanel({id:"swipenavigation",title:"Swipe Navigation",icon:"mdi:gesture-swipe-horizontal",content:Zi,active:U==="swipenavigation"})}
        ${this._renderSettingsPanel({id:"landing",title:"Landing Page",icon:"mdi:home-import-outline",content:Yi,active:U==="landing"})}
      </div>`,Qi=`<div id="config-save-reminder" class="config-save-reminder" role="status" aria-live="polite" aria-atomic="true" data-config-save-state="${this._hasConfigDraft===!0?"dirty":"clean"}">
        ...
        <div class="cam-modal-card camera-modal-card" role="dialog" aria-modal="true" aria-labelledby="camera-modal-title">
          ...
          <div id="camera-modal-group-fields" class="camera-group-fields" hidden>
            ...
            <div class="editor-choice-field camera-group-layout-field" role="radiogroup" aria-label="Live View Layout">
              ...
              ${ot({name:"camera-modal-group-layout",options:[{value:ye.sideBySide,label:"Side by Side"},{value:ye.stacked,label:"Stacked"}],selectedValue:ye.sideBySide})}
            </div>
          ...
          <div id="camera-modal-light-fields" class="camera-group-fields" hidden>
            ...
            <div class="editor-choice-field camera-group-layout-field" role="radiogroup" aria-label="Button Position">
              ...
              ${ot({name:"camera-modal-light-position",options:[{value:de.left,label:"Left"},{value:de.right,label:"Right"}],selectedValue:de.right})}
            </div>
          ...
          <div id="camera-modal-light-fields-2" class="camera-group-fields" hidden>
            ...
            <div class="editor-choice-field camera-group-layout-field" role="radiogroup" aria-label="Button Position">
              <div class="cam-modal-label">Button Position</div>
              ${ot({name:"camera-modal-light-position-2",options:[{value:de.left,label:"Left"},{value:de.right,label:"Right"}],selectedValue:de.right})}
            </div>
          ...
          <div class="cam-modal-field">
            ...
            <div id="camera-modal-ptz-rotation-row" hidden>
              <div class="editor-choice-field camera-group-layout-field" role="radiogroup" aria-label="PTZ Control Rotation">
                ...
                ${ot({name:"camera-modal-ptz-rotation",options:Ha.map(l=>({value:l,label:`${l}\xB0`})),selectedValue:0,compact:!0})}
              </div>
