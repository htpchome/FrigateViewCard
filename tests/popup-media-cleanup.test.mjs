...
test("recording reviews expose their first detection snapshot to the scrubber", async () => {
  const ctx = {
    _recordingAlertCache: new Map(),
    _browseWindowLoaderController: {
      async fetchWindowedReviews() {
        return [
          {
            id: "review-1",
            start_time: 120,
  ...
  const alerts = await FrigateViewCard.prototype._fetchRecordingAlerts.call(
    ctx,
    "client one",
    "front",
    100,
