...
test("PopupDragController touch start listener is passive", () => {
  const popup = createPopup();
  const controller = new PopupDragController({
    popup,
    eventTarget: createEventTargetLike(),
    closePopup: () => {},
    isPopupOpen: () => true,
  });
  ...
  assert.equal(popup.listenerOptions.get("touchstart")?.passive, true);
});
