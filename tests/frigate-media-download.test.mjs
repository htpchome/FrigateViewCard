...
test("Frigate review thumbnail paths preserve review and camera identity", () => {
  assert.equal(
    buildFrigateReviewThumbnailPath({
      clientId: "frig ate",
      reviewId: "review/1",
    ...
    "/api/frigate/frig%20ate/notifications/review%2F1/front%20door/review_thumbnail.webp",
  );
  assert.equal(
    buildFrigateReviewThumbnailPath({
      clientId: "frigate",
...
test("Frigate event downloads use the event camera context when provided", async () => {
  const downloads = [];
  const event = {
    id: "event-2",
    camera: "back_yard",
  ...
  const controller = new FrigateMediaDownloadController({
    getContext: () => ({ clientId: "front-instance", cam: "front_door" }),
    getContextForEvent: (value) =>
      value === event
        ? { clientId: "back-instance", cam: "back_yard" }
        : null,
    findEventById: () => event,
    download: (plan) => downloads.push(plan),
  });

  const plan = await controller.downloadEvent("event-2", "clip.mp4");

  assert.equal(
    plan.url,
    "/api/frigate/back-instance/notifications/event-2/clip.mp4?download=true",
  );
  assert.equal(plan.filename, "back_yard_event-2_clip.mp4");
  assert.deepEqual(downloads, [plan]);
});
