...
test("buildReviewFilterLabels combines objects and custom tags uniquely", () => {
  const labels = buildReviewFilterLabels(
    {
      data: {
        objects: ["person", "car", "person", ""],
        sub_labels: ["resident", "northern cardinal", "resident", ""],
      },
    },
    { label: "dog", sub_label: "known visitor" },
  );
...
test("collectFilterLabelsFromEvents gathers unique labels and custom tags", () => {
  const labels = collectFilterLabelsFromEvents([
    { label: "person", sub_label: "resident" },
    { label: "car", sub_label: "delivery driver" },
    { label: "person", sub_label: "resident" },
    {},
  ]);
...
test("selectFilterLabels uses event labels outside alerts tab", () => {
  ...
  assert.deepEqual(labels.sort(), ["car", "person", "resident"]);
});
...
test("toggleFilterSelection adds, removes, and clears multiple values", () => {
  assert.equal(toggleFilterSelection("all", "person"), "person");
  assert.deepEqual(toggleFilterSelection("person", "car"), ["car", "person"]);
  assert.equal(toggleFilterSelection(["car", "person"], "car"), "person");
  assert.equal(toggleFilterSelection("person", "person"), "all");
  assert.equal(toggleFilterSelection(["car", "person"], "all"), "all");
});

test("normalizeFilterSelections keeps available values from multi-selections", () => {
  assert.deepEqual(
    normalizeFilterSelections({
      filterLabel: ["person", "missing", "car"],
      filterZone: ["yard", "gone", "front"],
      labels: ["person", "car"],
      zones: ["yard", "front"],
    }),
    {
      filterLabel: ["car", "person"],
      filterZone: ["front", "yard"],
    },
  );
});
...
test("matchesEventFilters handles label, zone, and favorite checks", () => {
  ...
  assert.equal(
    matchesEventFilters(event, {
      filterLabel: ["car", "resident"],
      filterZone: ["driveway", "yard"],
      favOnly: true,
    }),
    true,
  );
...
test("matchesReviewFilters skips label and zone derivation when both are all", () => {
  assert.equal(
    matchesReviewFilters(
      { id: "review-1" },
      { retain_indefinitely: false },
      {
        getLabels: () => {
          throw new Error("labels should not be derived");
        },
        getZones: () => {
          throw new Error("zones should not be derived");
        },
      },
    ),
    true,
...
test("BrowseFilterController delegates filter state decisions from host state", () => {
  ...
  assert.deepEqual(controller.filteredActiveCameraReviews(), [reviewAlert]);

  host._filterLabel = "verified";
  host._filterZone = "all";
  assert.deepEqual(controller.filteredReviews(), [reviewAlert]);

  host._filterLabel = "all";
  host._filterZone = "all";
  assert.deepEqual(controller.filteredActiveCameraReviews(), [
    reviewAlert,
    reviewDetection,
  ]);
  host._activeCam.alerts_content = "alerts_only";
  assert.deepEqual(controller.filteredActiveCameraReviews(), [reviewAlert]);
  host._activeCam.alerts_content = "all_reviews";
  host._filterLabel = "resident";
  host._filterZone = "front";
...
test("BrowseFilterController handles filter panel interactions and rendering", () => {
  ...
  assert.deepEqual(JSON.parse(filterPanel.innerHTML), {
    labels: ["all", "person", "resident", "car"],
  ...
  assert.equal(
    controller.handleSidebarFilterClick({
      closest: (selector) =>
        selector === "[data-flabel]"
          ? { dataset: { flabel: "person" } }
          : null,
    }),
    true,
  );
  assert.deepEqual(host._filterLabel, ["car", "person"]);
  assert.deepEqual(calls, ["renderList"]);
  assert.deepEqual(JSON.parse(filterPanel.innerHTML).filterLabel, [
    "car",
    "person",
  ]);
  ...
  assert.equal(
    controller.handleSidebarFilterClick({
      closest: (selector) =>
        selector === "[data-fzone]"
          ? { dataset: { fzone: "driveway" } }
          : null,
    }),
    true,
  );
  assert.deepEqual(host._filterZone, ["driveway", "front"]);
  assert.deepEqual(calls, ["renderList"]);
  assert.deepEqual(JSON.parse(filterPanel.innerHTML).filterZone, [
    "driveway",
    "front",
  ]);
