...
export function slideshowReviewModeForCamera(config, entity) {
  const cam = config?.cameras?.find((camera) =>
...
export function cameraIndexForIncomingCamera(config, camCache, cameraId) {
  ...
  return (
    config?.cameras?.findIndex((camera) =>
      cameraMemberEntities(camera).some((entity, memberIndex) => {
        const discovered = String(camCache[entity]?.cam || "");
        const tokens = [
          entity,
          entity.replace(/^camera\./, ""),
          memberIndex === 0 ? camera?.name || "" : "",
          discovered,
...
export function cameraEntityForIncomingCamera(config, camCache, cameraId) {
  ...
  for (const camera of config?.cameras || []) {
    for (const [memberIndex, entity] of cameraMemberEntities(camera).entries()) {
      const tokens = [
        entity,
        entity.replace(/^camera\./, ""),
        memberIndex === 0 ? camera?.name || "" : "",
      ...
      if (tokens.includes(normalized)) return entity;
    }
  }
  return "";
}
