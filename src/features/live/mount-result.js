...
export const adoptMountedAttemptSlot = ({
  targetSlot,
  resultSlot,
  preservePendingSlots = false,
}) => {
  if (!targetSlot || !resultSlot) return;

  const currentChildren = [...(targetSlot.children || [])];
  const resultIsMounted =
    resultSlot.parentNode === targetSlot || currentChildren.includes(resultSlot);
  if (!resultIsMounted) {
    targetSlot.appendChild?.(resultSlot);
  }

  if (!preservePendingSlots) {
