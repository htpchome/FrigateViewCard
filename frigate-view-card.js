...
const FrigateViewCard = class extends HTMLElement {
  ...
  _applyCardStyle() {
    ...
    const tightMarginsEnabled = this._config?.tight_margins === true;
    const nonTightMarginHeightPenaltyPx = this._resolveNonTightMarginHeightPenaltyPx(tightMarginsEnabled);
  ...
  _resolveNonTightMarginHeightPenaltyPx(tightMarginsEnabled) {
    ...
    if (parentStyle) {
      penalty += px(parentStyle.marginTop) + px(parentStyle.marginBottom) + px(parentStyle.paddingTop) + px(parentStyle.paddingBottom);
    }
    let element = this;
    while (element) {
      if (element.tagName === "HUI-SECTIONS-VIEW") {
    ...
    if (this.isConnected && !this._nonTightPenaltyRetryRaf && this._nonTightPenaltyRetryCount < 2) {
