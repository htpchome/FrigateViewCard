...
const ICONS = {
  ...
  mobileView: '<svg viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="m6.9902 1c-0.5444 0-0.99023 0.44584-0.99023 0.99023v20.02c0 0.5444 0.44584 0.99023 0.99023 0.99023h10.02c0.5444 0 0.99023-0.44584 0.99023-0.99023v-20.02c0-0.5444-0.44584-0.99023-0.99023-0.99023h-10.02zm-0.49414 1.4004h11.008v19.199h-11.008v-19.199zm0.67383 0.40039c-0.23268 0-0.41992 0.18724-0.41992 0.41992v5.6602c0 0.23268 0.18724 0.41992 0.41992 0.41992h9.6602c0.23268 0 0.41992-0.18724 0.41992-0.41992v-5.6602c0-0.23268-0.18724-0.41992-0.41992-0.41992h-9.6602zm0 8c-0.23268 0-0.41992 0.18724-0.41992 0.41992v2.1602c0 0.23268 0.18724 0.41992 0.41992 0.41992h9.6602c0.23268 0 0.41992-0.18724 0.41992-0.41992v-2.1602c0-0.23268-0.18724-0.41992-0.41992-0.41992h-9.6602zm0 3.3984c-0.23268 0-0.41992 0.18724-0.41992 0.41992v2.1602c0 0.23268 0.18724 0.41992 0.41992 0.41992h9.6602c0.23268 0 0.41992-0.18724 0.41992-0.41992v-2.1602c0-0.23268-0.18724-0.41992-0.41992-0.41992h-9.6602zm0 3.6016c-0.23268 0-0.41992 0.18724-0.41992 0.41992v2.1602c0 0.23268 0.18724 0.41992 0.41992 0.41992h9.6602c0.23268 0 0.41992-0.18724 0.41992-0.41992v-2.1602c0-0.23268-0.18724-0.41992-0.41992-0.41992h-9.6602z" /></svg>',
...
function buildTabsMarkup({ tab, hiddenTabs, viewMode, icons: icons2 }) {
...
function buildToolsMarkup({
  ...
  const markup = `<div class="tl-tools" style=" margin-left: auto;">
        ${controlsHidden ? "" : `<button class="tool${tab === "controls" ? " active" : ""}" id="controls-btn" title="Controls" aria-label="Controls" aria-pressed="${tab === "controls" ? "true" : "false"}" ${controlsDisabled ? "disabled" : ""}>${icons2.bullseye}</button>`}
        ${gridButton}
        ${slideshowButton}
        <div class="divider">${icons2.divider}</div>
        <button class="tool${isFilterPanelOpen ? " active" : ""}" id="filter-btn" title="Filter" aria-pressed="${isFilterPanelOpen ? "true" : "false"}" ${resolvedFilterDisabled ? "disabled" : ""}>${icons2.filter}</button>
        <button class="tool${isCalendarPanelOpen ? " active" : ""}" id="cal-btn" title="Calendar" aria-pressed="${isCalendarPanelOpen ? "true" : "false"}" ${calendarDisabled ? "disabled" : ""}>${icons2.calendar}</button>
      </div>`;
  return markup;
}
...
function buildLiveEngineWrapMarkup({ icons: icons2, streamMuted }) {
  const muteLabel = streamMuted ? "Unmute live view" : "Mute live view";
  const muteIcon = streamMuted ? icons2.volOff : icons2.volOn;
  return `<div id="eng-wrap">
                <frigate-live-stream id="engine">
                  <div class="ph">${icons2.live}<span>Connecting\u2026</span></div>
                </frigate-live-stream>
                  <button class="glass-btn overlay-fs live-fs-btn" id="live-fs-btn" title="Fullscreen live" aria-label="Fullscreen live">${icons2.expand}</button>
...
function buildBrowseMarkup({ icons: icons2, layoutProfile = {} }) {
