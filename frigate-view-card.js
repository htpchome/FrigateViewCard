...
const ICONS = {
  ...
  mobileView: '<svg fill="currentColor" viewBox="0 0 24 24"><path d="M17,19H7V5H17M17,1H7C5.89,1 5,1.89 5,3V21A2,2 0 0,0 7,23H17A2,2 0 0,0 19,21V3C19,1.89 18.1,1 17,1Z" /></svg>',
...
// src/shared/media/picture-in-picture.js
...
function pictureInPictureElementForVideo(video, documentObj = null) {
  if (!video) return null;
  const root = video.getRootNode?.();
  return root?.pictureInPictureElement || resolveOwnerDocument(video, documentObj)?.pictureInPictureElement || null;
}
function isVideoPictureInPictureActive(video, documentObj = null) {
  if (!video) return false;
...
function resolveVideoPictureInPictureSupport({
  ...
  if (typeof video.requestPictureInPicture === "function" && doc?.pictureInPictureEnabled !== false) {
    return {
      supported: true,
...
const PictureInPictureButtonController = class {
  constructor({ button = null, video = null, documentObj = null } = {}) {
    __publicField(this, "_sync", () => this.refresh());
    this.button = button;
    this.video = video;
  ...
  refresh() {
    ...
    const mediaReady = this.video.readyState === void 0 || Number(this.video.readyState) > 0;
    const available = support.supported && mediaReady;
    const active = available && isVideoPictureInPictureActive(this.video, this._documentObj);
    ...
    this.button.title = active ? "Exit Picture-in-Picture" : "Picture-in-Picture";
...
const PopupCarouselSwipeController = class {
  constructor({
    ...
    requestFrame = globalThis.requestAnimationFrame?.bind(globalThis) || ((callback) => globalThis.setTimeout(() => callback(Date.now()), 16)),
    cancelFrame = globalThis.cancelAnimationFrame?.bind(globalThis) || globalThis.clearTimeout?.bind(globalThis),
    ...
    __publicField(this, "_onTouchEnd", (event) => this._finish(event, false));
    __publicField(this, "_onTouchCancel", (event) => this._finish(event, true));
    this._row = row;
...
const RecordingsBrowseNavController = class {
  ...
  async fetchRecordingsInBounds(bounds, clientId, cam, { forceRefresh = false } = {}) {
  ...
  prepareBrowseNav() {
    ...
    const alreadyResolved = !!key && key === this._lastBrowseNavKey && previous === this._lastBrowseNavPrevious && next === this._lastBrowseNavNext;
  ...
  scheduleBrowseNavUpdate() {
    ...
    const { previous, next } = this._browseNavNodes();
    const alreadyResolved = key === this._lastBrowseNavKey && previous === this._lastBrowseNavPrevious && next === this._lastBrowseNavNext;
    if (!key || key === this._scheduledBrowseNavKey || key === this._activeBrowseNavKey || alreadyResolved) {
      return false;
    }
...
function resolveDisplayedFrameSourceRect({
  ...
  const fitScale = fit === "cover" ? Math.max(viewportW / width, viewportH / height) : Math.min(viewportW / width, viewportH / height);
...
async function captureDisplayedFrame(media, {
  documentObj = globalThis.document,
  viewport = null,
  objectFit = "contain",
  zoomState = null,
  ...
  const sourceRect = resolveDisplayedFrameSourceRect({
    ...
    viewportWidth: positiveNumber(viewport?.width) || positiveNumber(parent?.clientWidth) || positiveNumber(media?.clientWidth),
    viewportHeight: positiveNumber(viewport?.height) || positiveNumber(parent?.clientHeight) || positiveNumber(media?.clientHeight),
    objectFit,
    zoomState
  });
...
function buildDisplayedFrameFilename({
  camera = "camera",
  capturedAt = new Date()
} = {}) {
  const safeCamera = String(camera || "camera").trim().replace(/[^a-z0-9._-]+/gi, "-").replace(/^-+|-+$/g, "") || "camera";
...
function downloadDisplayedFrame(blob, filename, {
  documentObj = globalThis.document,
...
function activateStandardPageRouteLifecycle({
  ...
  const useRetainedPreviewCamera = leavingPreview && context.startup !== true && context.deferCameraSwitch !== true;
...
const FrigateViewCard = class extends HTMLElement {
  ...
  _displayedSnapshotMedia(scope = "live") {
    if (scope === "popup") {
      const viewer = this._$("#viewer");
      return viewer?.querySelector?.("video") || viewer?.querySelector?.("img.snap") || null;
    }
  ...
  _displayedSnapshotCaptureOptions(scope, media) {
    const zoomController = scope === "popup" ? this._popupVideoZoomController : this._liveVideoZoomController;
    const activeZoomController = zoomController?.video === media ? zoomController : null;
    ...
    return {
      ...
      objectFit: computedStyle?.objectFit || media?.style?.objectFit || "contain"
    };
  }
  _showSnapshotResultBubble(scope, success) {
    const surface = scope === "popup" ? this._$("#viewer") : this._$("#live-stage");
    if (!surface) return;
    ...
    bubble.textContent = success ? "Snapshot taken" : "Unable to take snapshot";
  ...
  async _takeDisplayedSnapshot(scope = "live") {
    const button = this._$(
      scope === "popup" ? "#popup-take-snapshot-btn" : "#live-take-snapshot-btn"
    );
  ...
  async _togglePictureInPicture(video, { popup = false } = {}) {
    ...
    try {
      ...
      this._toast(
        reason ? `Picture-in-Picture could not start: ${reason}` : "Picture-in-Picture could not start in this browser."
      );
    } finally {
      this._syncPictureInPictureButtons();
